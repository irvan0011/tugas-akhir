...
//		return ResponseEntity.unprocessableEntity().body(apiError);
	...
	public ResponseEntity<Object> expiredJwtException(ExpiredJwtException ex, WebRequest request) {
		...
		LoggingFile.exceptionStringz(strExceptionArr, ex, OtherConfiguration.getFlagLoging());
		return buildResponseEntity(new ApiError(HttpStatus.FORBIDDEN,"Token Expired",ex,request.getDescription(false),"X-JWT-777"));
	}
	...
	public ResponseEntity<Object> unternalAuthenticationServiceException(InternalAuthenticationServiceException ex, WebRequest request) {
		return buildResponseEntity(new ApiError(HttpStatus.INTERNAL_SERVER_ERROR,"OTENTIKASI GAGAL",ex,request.getDescription(false),"X-AUTH256"));
	}
